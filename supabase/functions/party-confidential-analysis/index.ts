...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // İç çağrıda kullanıcı yok; kayıtlarda user_id yerine dosyanın arabulucusu kullanılır.
    ...
    const isPrivileged = isCron || !!roleRow || caseRow.user_id === userId || caseRow.assigned_mediator_id === userId;
...
KESİN KURAL (belgesiz durum): Belge sayısı sıfırsa (YÜKLENEN BELGELER: "(belge yok)"), document_findings[] dizisini BOŞ bırak ve hiçbir "belgede şu görülüyor/belirtiliyor" türü iddia üretme; yalnızca UYUŞMAZLIK ÖZETİ, TARAF BEYANI ve genel hukuki çerçeveye dayan.
