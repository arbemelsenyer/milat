/* ============ TASLAK DENETİMİ (İBA 2.6 / B21) — yalnız arabulucu ============
   Bu bileşen BELGE ÜRETMEZ ve METNİ DEĞİŞTİRMEZ. Üretilmiş taslağı okur, kalıpla
   bulunabilen belirsizlik/eksiklik/çelişki noktalarını bulunduğu satırla ve metinden
   tek cümlelik alıntıyla listeler. Düzeltmeyi arabulucu yapar.
   Belge üretim hattına (generate-official-document, OfficialDocumentsPanel'in üretim
   ve kaydetme akışı) DOKUNULMAZ — bu dosya ayrı durur, yalnız metni girdi alır.
   Hukuki geçerlilik yorumu YASAK: "geçersizdir/hükümsüzdür" denmez; yalnız neyin
   belirsiz, eksik ya da çelişkili göründüğü gösterilir (constitution m.2, m.5).
   Yeni AI çağrısı YOK, yeni tablo/sütun YOK. */
...
export function TaslakDenetimi({ caseId, metin }: { caseId: string; metin: string }) {
  ...
  async function denetle() {
    ...
    try {
      const { data, error } = await supabase.from("case_parties")
        .select("first_name, last_name, company_name, full_name")
        .eq("case_id", caseId);
      if (error) setHata(`Taraf kayıtları okunamadı, ad kontrolü yapılamadı: ${error.message}`);
      const adlar = taraflarinAdlari(Array.isArray(data) ? data : []);
      setBulgular(taslagiDenetle(metin, adlar));
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="rounded border bg-muted/20 p-3 space-y-2">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold uppercase tracking-wide">Taslak denetimi</span>
          {bulgular && bulgular.length > 0 && (
        ...
        <Button size="sm" variant="outline" className="h-8 text-xs" onClick={denetle} disabled={busy}>
      ...
      <p className="text-[11px] text-muted-foreground leading-snug">
        Denetim metni DEĞİŞTİRMEZ; yalnız belirsiz, eksik ya da çelişkili görünen yerleri gösterir.
        Hukuki geçerlilik değerlendirmesi yapmaz; düzeltmeyi siz uygularsınız. Yalnız size görünür.
      </p>

      {hata && <p className="text-[11px] text-destructive">{hata}</p>}

      {bulgular === null ? (
        <p className="text-[11px] text-muted-foreground italic">Denetim çalıştırılmadı.</p>
      ) : bulgular.length === 0 ? (
        <p className="text-xs">Denetimde eksiklik görünmüyor.</p>
      ) : (
        <ul className="space-y-2">
          {bulgular.map((b, i) => (
            <li key={i} className="text-xs border-l-2 border-amber-300 pl-2 space-y-0.5">
              <div className="font-medium">
                {b.tip} <span className="text-muted-foreground font-normal">· {b.konum}</span>
              </div>
              <div className="text-muted-foreground">{b.aciklama}</div>
              <div className="italic text-muted-foreground">“{b.alinti}”</div>
            </li>
          ))}
        </ul>
      )}
